...
public class HomeBanner extends FrameLayout implements View.OnClickListener {
    ...
    public void setImagesRes(int[] imagesRes, String[] title) {
        initLayout();
        initImgFromRes(imagesRes, title);
        // setAtt();

    }

    private void initImgFromRes(int[] imagesRes, String[] title) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            View banner_view = LayoutInflater.from(context).inflate(
                    R.layout.banner_content_layout, null);
            ImageView imageView_banner_title = (ImageView) banner_view
                    .findViewById(R.id.iv_banner_title);
            imageView_banner_title
                    .setScaleType(ImageView.ScaleType.CENTER_CROP);
            TextView textView_banner_title = (TextView) banner_view
    ...
    private void initImgFromNet(String[] imagesUrl) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            ...
            if (i == 0) {
                // mImageLoader.displayImage(imagesUrl[count - 1],
                // imageView_banner_title, options);
            } else if (i == count + 1) {
                // mImageLoader.displayImage(imagesUrl[0],
                // imageView_banner_title, options);
            } else {
                // mImageLoader.displayImage(imagesUrl[i - 1],
                // imageView_banner_title, options);
            }
            views.add(banner_view);
        }
    ...
    //
    // if (i == 0) {
    // mImageLoader.displayImage(topBannerEntities.get(len - 1)
    // .getImage(), imageView_banner_title, options);
    // textView_banner_title.setText(topBannerEntities.get(len - 1)
    // .getTitle());
    // } else if (i == len + 1) {
    // mImageLoader.displayImage(topBannerEntities.get(0).getImage(),
    // imageView_banner_title, options);
    // textView_banner_title.setText(topBannerEntities.get(0)
    // .getTitle());
    // } else {
    // mImageLoader.displayImage(topBannerEntities.get(len - 1)
    // .getImage(), imageView_banner_title, options);
    ...
    Runnable runnable = new Runnable() {
        ...
        public void run() {
            if (isAutoPlay) {
                // currentItem = currentItem % (topBannerEntities.size() + 1) +
                // 1;
    ...
    public void onClick(View v) {
        if (onItemClickListener != null) {
            TopBannerEntity entity = topBannerEntities.get(viewPager
                    .getCurrentItem() - 1);
